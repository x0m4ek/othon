...
import MobileServices from "../Sliders/MobileServices.tsx";
...
const Services: FC = () => {
    ...
    const isMobile = useMediaQuery({ query: '(max-width: 1420px)' })
    const cats = [
    ...
    const selectedCatData = data_for_cat.find(cat => cat.title === catSelected);
                    ...
                    <div className={styles.options}>
                        {cats.map((cat, index) => (
                            <Cat
                                ...
                                onClick={() => handleCatClick(cat, index)}
                            />
                        ))}
                    </div>
                    <div className={styles.slider}>
                        <Slider images={images} ref={sliderRef} />
                    </div>
                    {selectedCatData && (
                        <AnimatePresence>

                            <motion.div className={styles.services_selected}
                                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}
                            >
                                <p>{selectedCatData.text}</p>
                            </motion.div>
                        </AnimatePresence>
                    )}
                </div>
